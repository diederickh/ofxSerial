        ...
        : pimpl(new AsyncSerialImpl)
{
	try {
		...
		std::cout << "AsyncSerial: cannot open!" << std::endl;
	}
}
...
void AsyncSerial::doClose()
{
    ...
    if(pimpl->port.is_open()) pimpl->port.close(ec);
    pimpl->io.reset();
        ...
        asio::serial_port_base::stop_bits opt_stop)
{
	...
	try {
		...
		std::cout << "Error while opening serial port: " << devname << std::endl;
		//exit(0);
		return;
	}
...
AsyncSerial::~AsyncSerial()
{
    ...
    {
        try {
            //close(); // @note diederick: removed this; keeps the app open.
        } catch(...)
        {
            //Don't throw from a destructor
			cout << "??????";
